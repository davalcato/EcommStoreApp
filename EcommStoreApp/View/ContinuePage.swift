...
struct ContinuePage: View {
    ...
    @State private var itemsSubtotal: Double = 0.0

    var body: some View {
        ScrollView {
            VStack {
                Spacer().frame(height: 20) // Add some padding at the top
    ...
    func calculateTotalBeforeTax() -> Double {
        return sharedData.cartProducts.reduce(0.0) { $0 + (Double($1.price) ) }
    }

    func calculateItemsSubtotal() -> Double {
        // This function should also sum the prices of the products
        return sharedData.cartProducts.reduce(0.0) { $0 + ($1.price * Double($1.quantity)) }
        }
}
